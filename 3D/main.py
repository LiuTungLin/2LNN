...
if __name__ == "__main__":
    ...
    train_loader, test_loader, input_size = prepare_data('training_data_3D.xlsx')

    np.random.seed(1)
    torch.manual_seed(1)
    torch.cuda.manual_seed_all(1)

    model = TwoLayerNet(input_size=input_size, hidden_size=32).to(device)
    ...
    train_hist, test_hist, result = train_model(
        ...
        loss_threshold=0.6, max_t=50, check_interval=5
    )

    evaluate_model(model, 'validation_data_3D.xlsx')
    if result == False:
        for i in range(5):
            print(f"\n第 {i+1} 次 Select")
            selected_idx, x_k, y_k = selecting_LTS(model, train_loader, epsilon=2.5)
            if x_k is None:
            ...
            scheduler = lr_scheduler.StepLR(optimizer, step_size=5, gamma=1.2)
            train_model(
                model, train_loader, test_loader,
                criterion, optimizer, scheduler,
                loss_threshold=0.55, max_t=50, check_interval=5
            )
        ...
        train_model(
            ...
            criterion, optimizer, scheduler,
            loss_threshold=0.59, max_t=50, check_interval=5
        )
        ...
        print("\nmodel_dropout")
        evaluate_model(model_dropout, 'validation_data_3D.xlsx')

        print("\nNode-pruning_AN")
        model_pruned = prune_nodes(model_dropout, train_loader, test_loader, criterion)

        print("\nmodel_pruned")
        evaluate_model(model_pruned, 'validation_data_3D.xlsx')
